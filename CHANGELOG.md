...
- Added a "Related Projects And Adjacent Standards" section to the README, with Spectrum as a project focused on keeping the original source intact, and a note positioning Vector Passport as a portable standard alongside platforms with their own native provenance such as Vectara and HydraDB, since each platform does provenance its own way and a shared convention is what lets vectors move between them.
- Rewrote the README opening to lead with the standard framing, including a "Where this should go" note that the real endgame is provenance being understood natively by vector stores, with the passport carrying the standard until that happens.
- Added Dependabot for pip and GitHub Actions, skipped CI on funding and prose-only paths, pinned Ruff, and added `SECURITY.md`.
